...
module.exports.request = function(api, method = "GET", options = {}) {
  const uri1 = getCdnFullUrl(api);
  return this._request(uri1, method, options).catch(err => {
    this.logger.warn("请求失败，切换CDN重试", err);
    // 如果并发时，其他请求失败，导致已切换cdn
    // 那么当前请求直接使用切换后的cdn即可
    if(uri1 === getCdnFullUrl(api)) {
      cdn.switchCdn();
    }
    return this._request(getCdnFullUrl(), method, options);
  });
};
...
function getCdnFullUrl(api) {
  return resolveUrl(api, cdn.getOrigin());
}
